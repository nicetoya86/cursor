...
**추출 우선순위:**
1. HTML div 패턴 (type="end-user")에서 고객 직접 입력 추출
2. "고객 문의 내용:" 패턴 명시적 내용 추출  
3. 시간 스탬프 기반 고객 발언 추출
4. 고객명 기반 필터링
5. 일반 필터링 (제외 패턴 적용)
...
- 자동 응답: 안녕하세요, 여신티켓입니다, 문의 주셔서 감사합니다, 빠른 시일 내, 답변 드리겠습니다
- 운영 안내: 운영시간 안내, 평일, 주말 공휴일 휴무, 점심시간, 순차적으로 처리, 양해 부탁
- 기술적 내용: 본인확인, 인증번호, 구매 ID, zendesk.com, 업로드함, URL:, 유형:
- 파일 관련: 파일 업로드, 첨부파일, attachment, upload, download, .png, .jpg, .jpeg, .pdf, .doc, .docx
- WEB 발신: [Web발신], Web발신, 발신전화, 부재중, 수신전화, LMS 전송, SMS 전송, MMS 전송
- 웹 관련: WEB 발신, 웹 발신, WEB발신, 웹발신, [WEB 발신], [웹 발신], WEB, 웹
- 문서 관련: [도움말], [FAQ], [가이드], [매뉴얼], [안내], [문서], 관련 문서:, 참고 문서:
- URL/웹사이트: http://, https://, www., .com, .co.kr
- 개인정보: 인증번호, 휴대전화, 연락처, 전화번호, 6자리 숫자, 모든 지역번호 패턴
- 기타: 피부 시술, 일상, 여신티켓, 마이페이지, hcaptcha, Screenshot_, play play
- 최소 길이: 3자 미만 텍스트 제외

**태그 정보:** ${tagList || '없음'}

**출력 방식:**
- 위 우선순위에 따라 고객 문의 내용 추출
- 여러 문의가 있으면 모두 포함하되 중복 제거
- 구체적인 문의가 없으면 태그 기반으로 문의 유형 추정
- 예: "병원 서비스 불만 관련 문의", "결제 전환취소 관련 문의"
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "당신은 고객 서비스 티켓 분석 전문가입니다. 제시된 5단계 우선순위 기준과 상세한 제외 패턴을 정확히 적용하여 고객의 실제 문의 내용만을 추출해주세요. 80개 이상의 제외 조건을 엄격히 적용하되, 고객의 진짜 문의는 놓치지 마세요."
        },
        {
          role: "user",
          content: prompt
