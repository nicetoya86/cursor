...
// GPT 프롬프트 템플릿 (프로젝트 기준에 따른 문의 내용 추출)
const createExtractionPrompt = (ticketContent, tags) => {
  const tagList = Array.isArray(tags) ? tags.join(', ') : tags;
  
  return `다음 고객 서비스 티켓에서 고객이 실제로 작성한 문의 내용을 추출해주세요.

**추출 우선순위 (프로젝트 기준):**
1. **HTML div 패턴**: type="end-user"인 div에서 고객 직접 입력 추출
2. **"고객 문의 내용:" 패턴**: 명시적으로 표시된 고객 문의 내용
3. **시간 스탬프 기반**: (시간) 발신자: 내용 형태에서 고객 발언만 추출
4. **고객명 기반**: "○○님과의 대화"에서 해당 고객명 발언만 추출
5. **일반 필터링**: BOT/매니저/시스템 메시지 제외 후 고객 내용만

**제외 대상 (프로젝트 기준):**
- 상담원: 여신BOT, 매니저L, 매니저B, 매니저D, Matrix_bot
- 시스템 메시지: 해결되었어요, 도움이 되었어요, 선택해 주세요
- 자동 응답: 안녕하세요, 문의 주셔서 감사합니다, 빠른 시일 내 답변
- 기술적 내용: URL, 파일 업로드, 첨부파일, 인증번호, 전화번호
- WEB 발신: [WEB 발신], 웹 발신 관련 모든 메시지
- 파일 관련: .png, .jpg, .pdf, 파일명:, 크기:, 용량:

**태그 정보:** ${tagList || '없음'}

**출력 방식:**
- 위 우선순위에 따라 고객 문의 내용을 찾아서 원문 그대로 출력
- 여러 문의가 있으면 모두 포함하되 중복 제거
- 실제 문의 내용이 없으면: "명확한 문의 내용이 확인되지 않습니다."
